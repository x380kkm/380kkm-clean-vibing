// audience: internal
// # preread-reminder
// 用户级 PreToolUse hook(匹配 Bash,shell_command):读码前从 AGENTS.md 注入 preread 块(用 cleanread 读,索引
// 生命周期,reuse)外加 plain 块.索引的怎么用和读前刷新由 preread 注入,收尾的全量重建由 stop-reminders
// 负责.按 session 记时间戳节流.内容全部来自 AGENTS.md,读不到则不注入.
...
//// 从 AGENTS.md 组装 preread 场景注入文本(场景块加去重后的 plain);为空则不注入 [@380kkm 2026-06-22] ////
const ctx = composeInjection("preread");
if (!ctx) emit({});
//// /取注入内容 ////

//// 节流:同 session 距上次注入不到 THROTTLE_MS 就放行 [@380kkm 2026-06-22] ////
const stampFile = path.join(os.tmpdir(), `codex-preread-${sessionId}.stamp`);
const now = Date.now();
let last = 0;

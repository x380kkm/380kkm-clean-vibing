...
// 不变量一:嵌套 codex 子进程(CODEX_HOOK_NESTED=1)直接退出,断开递归.
...
// //// 防递归:嵌套 codex 子进程直接退出 [@380kkm 2026-06-15] ////
if (process.env.CODEX_HOOK_NESTED === "1") done();
// //// /防递归:嵌套 codex 子进程直接退出 ////

// //// 读取输入与会话状态 [@380kkm 2026-06-15] ////
let input = {};
try { input = JSON.parse((fs.readFileSync(0, "utf8") || "{}").replace(/^\uFEFF+/, "")); } catch { done(); }

...
# audience: internal
# ref
"""cleanread L4 —— ref、prune、worktree 命令行（仅标准库）。

规范第 10 节。一个 *ref* 是带日期、按任务打标签的阅读工作区，存放选定文件的
经裁剪 + 标注的副本。它位于
  <root>/.cleanread/refs/<id>/      其中  id = <YYYY-MM-DD>-<task-slug>
...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    if rc != 0:
        ...
        return rc
    # 符号富化失败时，ref 子索引仍标记为已构建，不中止整体流程。
    ...
    sub_db = (index_root / "cleanread" / "source.db")
    if sub_db.exists():
        manifest["sub_index"] = (Path("files") / "cleanread" / "source.db").as_posix()
        save_manifest(ref_dir, manifest)
...
def _add_locator(sp: argparse.ArgumentParser) -> None:
    sp.add_argument("--store", default=None, help="explicit cleanread store dir (default: discover)")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="ref.py", description="cleanread L4 ref/prune CLI")

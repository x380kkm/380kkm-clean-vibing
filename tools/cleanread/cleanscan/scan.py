...
# audience: internal
# cleanscan.scan
"""cleanscan CLI —— 对 cleanread 存储库做有界、按需驱动的依赖扫描。

提供各子命令，可直接调用工具而无需手写查询：
...
from lib import agentapi, analyze, boundary, cache, render, rollup, scope, stores  # noqa: E402
...
#### 扫描 seed 切片，节点为空时告警，按需做 rollup，返回最终图 [@380kkm 2026-06-05] ####
...
#### impact 子命令：改动 seed 会牵连谁（面向代理的精简答案） [@380kkm 2026-06-16] ####
def cmd_impact(args) -> int:
    info = _store_info(args)
    with stores.Store(info.db_path) as st:
        ans = agentapi.impact(st, args.seed, top=args.top)
    _emit(json.dumps(ans, ensure_ascii=False, indent=2))
    return 0
#### /cmd_impact ####


#### 缺 --target-root 且无模块标记时拒绝自动判定，返回退出码或 None [@380kkm 2026-06-16] ####
def _require_target_root(args, st) -> int | None:
    if args.target_root is None and not boundary.has_module_markers(st):
        print("error: cannot autodetect --target-root (no *.uplugin / *.Build.cs markers are "
              "indexed). Pass --target-root <dir> explicitly (or --target-root \"\" to treat "
...
#### interface 子命令：这块代码对外依赖的接口（面向代理的精简答案） [@380kkm 2026-06-16] ####
def cmd_interface(args) -> int:
    info = _store_info(args)
    with stores.Store(info.db_path) as st:
        rc = _require_target_root(args, st)
        if rc is not None:
            return rc
        ans = agentapi.interface(st, args.target_root, args.dep_root, info.alias,
                                 max_nodes=args.max_nodes, depth=args.depth, top=args.top)
    _emit(json.dumps(ans, ensure_ascii=False, indent=2))
    return 0
#### /cmd_interface ####


#### seam 子命令：seed 邻域里的切点（面向代理的精简答案） [@380kkm 2026-06-16] ####
def cmd_seam(args) -> int:
    info = _store_info(args)
    with stores.Store(info.db_path) as st:
        ans = agentapi.seam(st, args.seed, info.alias,
                            max_nodes=args.max_nodes, depth=args.depth)
    _emit(json.dumps(ans, ensure_ascii=False, indent=2))
    return 0
#### /cmd_seam ####
...
#### boundary 子命令：构建并渲染符号级 target 与 dependency 边界 [@380kkm 2026-06-05] ####
...
#### 从 --modules / --module / cleanread.json 解析出 N 路模块规格 [@380kkm 2026-06-05] ####
def _resolve_module_spec(args, info):
    """优先级：--modules 文件（含 --module 内联合并）> cleanread.json['modules']（含内联合并）。
...
def cmd_modules(args) -> int:
    ...
    if spec is None:
        print("error: no module spec given. Provide --modules <file>, one or more "
              "--module NAME=PREFIX[,PREFIX...], or a cleanread.json['modules'] block.",
              file=sys.stderr)
        return 2
    with stores.Store(info.db_path) as st:
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="cleanscan",
        description="bounded, demand-driven dependency scans over cleanread stores",
    )
    sub = ap.add_subparsers(dest="cmd", required=True)

    sub.add_parser("list-stores", help="list hub-registered cleanread stores").set_defaults(func=cmd_list)
    ...
    pb.add_argument("--ignore", default=None,
                    help="(html only) view-hide config JSON (names/patterns/min_fan_in, or a "
                         ...
                         "cleanread.json['view_hide']. Absent + no config => identical to v0.6.0.")
    ...
    #### 面向代理的精简 API：impact、interface、seam（输出紧凑 JSON，带 path:line 锚点） [@380kkm 2026-06-16] ####
    im = sub.add_parser("impact", help="agent API: who depends on a symbol (reverse deps), ranked + anchored")
    im.add_argument("seed", help="symbol / file / dir to assess the change-impact of")
    _add_store_args(im)
    im.add_argument("--top", type=int, default=25, help="cap the listed dependents (rest summarized)")
    im.set_defaults(func=cmd_impact)

    it = sub.add_parser("interface", help="agent API: the dependency surface of a target (anchored + confidence)")
    _add_store_args(it)
    it.add_argument("--target-root", dest="target_root", default=None,
                    help="target root prefix (\"\" = whole index is the target)")
    it.add_argument("--dep-root", dest="dep_root", action="append", default=[],
                    help="dependency root hint (repeatable)")
    it.add_argument("--max-nodes", dest="max_nodes", type=int, default=200)
    it.add_argument("--depth", type=int, default=4)
    it.add_argument("--top", type=int, default=40, help="cap the listed interface entries")
    it.set_defaults(func=cmd_interface)

    se = sub.add_parser("seam", help="agent API: cut points (bridges + articulation nodes) near a seed")
    se.add_argument("seed", help="symbol / file / dir to find split seams around")
    _add_store_args(se)
    se.add_argument("--max-nodes", dest="max_nodes", type=int, default=80)
    se.add_argument("--depth", type=int, default=6)
    se.set_defaults(func=cmd_seam)
    #### /面向代理的精简 API ####
    ...
    md.add_argument("--modules", default=None,
                    help="module spec file ({version,fallback,zones} or a {modules:{...}} "
                         "wrapper). Default: auto-discover cleanread.json['modules'].")

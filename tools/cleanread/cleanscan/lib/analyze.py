...
# ///
# audience: internal
# cleanscan.lib.analyze
"""cleanscan.lib.analyze —— 依赖 Graph 之上的重构支持度量（纯函数）。

给定任意一个 :class:`graph.Graph`（一个有界或已 rollup 的切片），计算团队推理
模块化与重构所需的信号，且从不改动该 graph：

  * 逐节点耦合：``fan_in``/``fan_out`` 以及 Martin 的 ``Ca``/``Ce``/``instability``
    （``Ce/(Ca+Ce)``；0 = 稳定，1 = 不稳定）。
  * ``cycles``    —— 强连通分组（>1 个节点），必须打破才能解耦。
  * ``bridges``   —— 移除后会切开 graph 的边（移除可拆分图的候选边）。
...
#### 数无向图的连通分量数，可选跳过某节点、或移除某条边 [@380kkm 2026-06-05] ####

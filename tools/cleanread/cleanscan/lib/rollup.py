...
# ///
# audience: internal
# cleanscan.lib.rollup
"""cleanscan.lib.rollup —— 把文件级切片折叠到目录、模块层级。

在 :func:`graph.rollup` 之上按目录、或按 *module*（最近的带构建标记文件如
``CMakeLists.txt`` / ``*.Build.cs`` / ``pyproject.toml`` / ``*.uplugin`` 的祖先目录）
分组，并把有界扩展的计量（``truncated`` / ``frontier`` / ``elided`` /
``depth_bounded``）带到折叠后的图上，每个被省略的边界计数归属到它所属的分组。
"""
...
#### 把图折叠到目标层级，并传递有界扩展计量 [@380kkm 2026-06-05] ####
def rollup(g: Graph, level: str, store: "stores.Store | None" = None) -> Graph:
    """``file`` = 恒等；返回图带 `g` 的有界扩展计量，被省略的边界计数重归属到源节点所属分组。"""
    if level == "file":
        return g

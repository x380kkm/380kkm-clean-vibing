...
# ///
# audience: internal
# cleanscan.lib.boundary.modules_views
"""cleanscan.lib.boundary.modules_views —— N 路模块分区图之上的解耦派生视图。

全部函数对图做纯数据变换，除有界 build 外不再查库，包括：边的内部、跨模块分类，NxN 区矩阵，
按有序模块对列出的所需符号（附证据、引用计数及切割代价权重），模块级环检测（在 N 节点商图上
用迭代 Tarjan 算法），每模块扇入、扇出与不稳定度，以及按模块对汇总的最低切割代价。
"""
...
#### 取节点的模块名，即 attrs['zone']，由构建阶段写入 [@380kkm 2026-06-05] ####
...
#### A 从 B 需要的一个目标符号（A 对 B 的一个跨模块依赖点） [@380kkm 2026-06-05] ####
...
class Need:
    ...
    ref_count: int
    # 该 dst 节点命中的最长 include 前缀（用于调试规格重叠，无匹配则为空串）
    winning_prefix: str


#### 取某路径在规格下命中的最长 include 前缀，用于调试规格重叠 [@380kkm 2026-06-05] ####
def winning_prefix(path: str | None, spec: ModuleSpec) -> str:
    m = match_path(path, spec)
    return m[0] if m is not None else ""


#### 按 (A,B,dst) 累积跨模块边证据，包括 relations、引用源、label、path、最长匹配前缀 [@380kkm 2026-06-05] ####
...
#### 按有序模块对 (A,B) 列出 A 从 B 需要的符号（附证据与引用计数） [@380kkm 2026-06-05] ####
...
def module_cycles(g: Graph) -> list[list[str]]:
    """构造 N 节点商图，每个已声明模块对应一个节点，A->B 当且仅当存在跨模块边 A->B，
    然后用迭代 Tarjan 算法求强连通分量。任何 size>1 的强连通分量即一个模块环。N 为模块数
    而非符号数，规模有界。
    """
    quo = Graph()
...
class ModuleFanStat:
    ...
    fan_in: int
    fan_out: int
    # I = fan_out / (fan_in + fan_out)，与 analyze 对文件的计算方式一致；无边时为 0.0
    instability: float
...
#### 一个有序模块对 (A,B) 的切割代价，等于 A 从 B 需要的去重符号数 [@380kkm 2026-06-05] ####
@dataclass(frozen=True)
class CutCost:
    src_module: str
    dst_module: str

...
# ///
# audience: internal
# cleanscan.lib.boundary.build
"""cleanscan.lib.boundary.build — 深度 1 依赖汇（dependency sink，只汇入不展开的节点）构建流程。

展开深度 1 的依赖汇视图：每个目标符号加上其单层依赖边界（带置信度解析），
无论依赖索引多大，都通过直接构造图完成。
"""
...
def build(store, z: Zoning, budget: Budget, alias: str | None = None,
    ...
    依赖区或 ``dep:`` 目标作为深度 1 依赖汇节点加入。``budget.max_nodes`` 是带截断的安全上限，
    每条边的置信度记入 ``g.edge_confidence``，UE ``*_API`` 宏被跳过。

    ``dep_depth`` 控制目标之外展开的有界出边层数。``dep_depth <= 1``（默认）只展开依赖第一层，
    每个依赖节点都是汇节点。``dep_depth >= 2`` 在第一层之后再展开一层，该层首次加入的
    依赖 SYMBOL 节点标记 ``dep_core``。``truncated`` 与 ``elided`` 计数器跨两轮叠加。
    """
    g = Graph()

...
# ///
# audience: internal
# cleanscan.lib.boundary.resolve
"""cleanscan.lib.boundary.resolve —— 带可靠性置信度的边解析。

把每条符号边（``extends`` / ``implements`` / ``uses_type``）解析到一个具体目标，并附带
可靠性置信度（绝不在多个同名候选中悄悄挑一个），另外提供只读的存储库出边查询函数
（:func:`out_edges`）及其关系集合（:data:`REL`）。
"""
...
def resolve_target(store, row, z: Zoning, alias: str | None = None) -> Resolved:
    """若设置了 ``dst_symbol_id``，直接返回该符号，置信度为 ``direct``。
    否则按精确名在全局解析 ``dst_name``：

    - 0 个候选：返回 ``dep:<name>``，置信度为 ``unresolved``。
    - 恰好 1 个候选：返回该符号，置信度为 ``unique``。
    - N > 1 个候选：返回 ``dep:<name>`` 且 ``ambiguity=N``，置信度为 ``ambiguous``。
    """

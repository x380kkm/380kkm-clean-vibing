...
# ///
# audience: internal
# cleanscan.lib.boundary.zoning
"""cleanscan.lib.boundary.zoning — 将已索引符号划分为 TARGET 区与 DEPENDENCY 区。

本模块把已索引的符号分配到 TARGET（正在分析的代码）与 DEPENDENCY 区
（被分析代码所依赖的部分，可能来自多个来源），提供 `boundary` 包其余部分所需的路径分类工具，
并生成依赖侧的可读标签。
"""
...
class Zoning:
    ...
    ``dep_roots`` 仅作依赖侧标注或分组提示，不改变目标判定（符号为依赖当且仅当不在
    ``target_root`` 之下）。
    """
...
def zone_of_path(path: str | None, z: Zoning) -> str:
    """规范化路径等于 ``target_root`` 或以 ``target_root + '/'`` 开头时归为 TARGET；``target_root == ""`` 时全部路径归 TARGET；缺失路径归 DEPENDENCY。"""
    if path is None:
        return DEPENDENCY
    p = _NORM(path)
    pr = z.target_root

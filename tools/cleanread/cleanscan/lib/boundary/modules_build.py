...
# ///
# audience: internal
# cleanscan.lib.boundary.modules_build
"""cleanscan.lib.boundary.modules_build —— N 路模块分区的有界构建流水线（build 的并行体）。

按声明模块的路径前缀逐 zone 播种（LIKE 预筛 + ESCAPE，``module_of_path`` 为权威），把每个
被达符号建为带 ``module`` 属性的一等节点，跟随其边界出边解析；落入默认分类（External）的符号是
汇点（不展开），声明模块内的符号在预算内可继续展开。与 ``build`` 不同，没有单一「依赖汇」——
任意声明模块都是一等区。``dep_depth`` 控制默认分类之后的额外有界展开层（沿用 ``build`` 语义）。
"""
...
#### 直接构造 N 路模块分区图：声明模块为一等区，默认分类为汇点 [@380kkm 2026-06-05] ####
def build_modules(store, spec: ModuleSpec, budget: Budget, alias: str | None = None,
    ...
    解析。目标节点落入声明模块（一等，可在预算内继续展开）或默认分类 External（汇点）。
    ``budget.max_nodes`` 是带 ``truncated``、``elided`` 计量的硬上限，逐边置信度记于
    ``g.edge_confidence``，UE ``*_API`` 宏被跳过。``dep_depth >= 2`` 时在默认分类表层后多展开一层，
    新加入的默认分类符号标 ``dep_core``。
    """
    g = Graph()
    ...
    #### dep_depth>=2：默认分类表层之后多展开一层，标记 dep_core [@380kkm 2026-06-05] ####
    if dep_depth >= 2:
...
def _expand_layer(g: Graph, store, spec: ModuleSpec, alias, cap: int, confidence: dict,
    ...
    对每条加边的目标调用 ``collect(node, was_new)``，由调用方决定收集什么。返回 ``(本次 elided 数,
...
#### 从默认分类表层符号多展开一层依赖，返回 (id, 是否首次加入) 列表 [@380kkm 2026-06-05] ####

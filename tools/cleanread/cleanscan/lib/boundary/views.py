...
# ///
# audience: internal
# cleanscan.lib.boundary.views
"""cleanscan.lib.boundary.views —— boundary graph 之上的派生 VIEW。

两个派生 view 及其分层和折叠呈现：
  * INTERNAL coupling —— target 区子图（仅 target 到 target 边），用于定位分割点、分析强连通分量（SCC）。
  * DEPENDENCY surface —— 二部 boundary（够到某个 dependency 的 target 符号），映射到它们的 dependency target，可选按 module rollup。
此外还有 band 分配（4 层重构 view）、module 分配（可折叠的商图 view），以及
target 到 dependency 的 crossing 列表。
"""
...
#### 取 target 区子图（仅 target 到 target 边） [@380kkm 2026-06-05] ####
...
#### 取二部 boundary 表面：target boundary 符号到其 dependency target [@380kkm 2026-06-05] ####
def dependency_surface(g: Graph, rollup_modules: bool = False, store=None) -> Graph:
    """只保留跨越（target 到 dependency）边及其端点。设置 ``rollup_modules`` 时，dependency
...
#### 一次 target 到 dependency boundary crossing（target 侧引用 dependency 侧的单条跨越记录） [@380kkm 2026-06-05] ####
...
#### 取所有 target 到 dependency crossing，按 (src, dst, relation) 排序 [@380kkm 2026-06-05] ####

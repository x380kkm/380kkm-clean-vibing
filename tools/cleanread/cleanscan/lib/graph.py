...
# audience: internal
# cleanscan.lib.graph
"""cleanscan.lib.graph —— 内存中的节点、边模型和图算法。

纯函数、无 IO：调用方注入 ``expand`` 回调产出邻居，算法只操作得到的
:class:`Graph`。:func:`bfs_bounded` 在 ``budget.max_nodes``、
``budget.max_depth`` 处停止，返回至多持有 ``max_nodes`` 个节点的图。
"""
...
#### 有向边 ``src -> dst``，携带关系、凭证和权重 [@380kkm 2026-06-05] ####
@dataclass
class Edge:
    # node id
    src: str
...
#### :func:`bfs_bounded` 的硬边界 [@380kkm 2026-06-05] ####
@dataclass(frozen=True)
class Budget:
    max_nodes: int = 200
    max_depth: int = 3
...
class Graph:
    ...
    def add_edge(self, edge: Edge) -> bool:
        """两端必须已作为节点存在；重复边将权重累加到已有边，而不新增平行边。"""
...
#### 逐层完整的有预算 BFS [@380kkm 2026-06-05] ####
def bfs_bounded(
    seed_nodes: Iterable[Node],
    expand: Callable[[str], Iterable[Step]],
    budget: Budget,
) -> Graph:
    """从 ``seed_nodes`` 出发、经 ``expand(id)->Steps`` 逐层完整展开。

    若纳入整个下一层会超过 ``budget.max_nodes``，则该层整体被拒，边界记于：

      * ``truncated``      —— 因节点预算拒掉某一层时为 True。
      * ``frontier[id]``   —— 各源在边界处被省略的出向依赖计数。
      * ``elided``         —— 边界处被拒的不重复节点总数。
      * ``depth_bounded``  —— 在 ``max_depth`` 处停止扩展时为 True。
      * ``frontier_depth`` —— 完整纳入的最深层级。

    保证 ``len(result) <= budget.max_nodes``；被拒边界处已纳入节点间的闭合边仍记录。
    """
    g = Graph()
    frontier: list[str] = []
    for n in seed_nodes:
...
def toposort(graph: Graph) -> tuple[list[str], list[str]]:
    """``leftover`` 是仍带入向边的节点 id，即参与环或处于环下游的那些。"""
...
def scc(graph: Graph) -> list[list[str]]:
    """返回分量列表（每个为排序后的节点 id）；>1 节点或带自环的单节点即为环。"""
...
def rollup(graph: Graph, group_of: Callable[[Node], str],
    ...
    """组内边被丢弃，跨组边权重相加；分组节点 kind 为 ``group``，带 ``attrs['members']`` 计数。"""
    g = Graph()

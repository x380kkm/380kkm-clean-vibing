...
# ///
# audience: internal
# cleanscan.lib.stores
"""cleanscan.lib.stores —— 对 cleanread 存储库的只读访问。

cleanscan 构建于 cleanread 之上。它从不重新声明 cleanread 的 schema 或存储布局；而是
按文件路径（以别名化的模块名）加载 cleanread 自己的 ``lib/config.py`` 与 ``lib/db.py``，
这样 cleanread 对其存储模型所做的任何改动都会自动传导到这里。对存储库 ``source.db``
...
#### 解析 cleanread 插件的 scripts/ 目录（兼容层文件所在位置） [@380kkm 2026-06-05] ####
def cleanread_scripts_dir() -> Path:
    """优先遵从 ``CLEANSCAN_CLEANREAD`` 覆盖，否则取缓存 glob 中版本号最高的已安装插件。"""
    # 同仓内 cleanread 的 lib 位于 parents[2]/lib
    in_plugin = Path(__file__).resolve().parents[2]
    if not os.environ.get("CLEANSCAN_CLEANREAD") and (in_plugin / "lib" / "config.py").is_file():
        return in_plugin

    env = os.environ.get("CLEANSCAN_CLEANREAD")
    if env:
        p = Path(env)
        cand = p / "scripts" if (p / "scripts").is_dir() else p
        if (cand / "lib" / "config.py").is_file():
            return cand
        raise FileNotFoundError(f"CLEANSCAN_CLEANREAD={env} has no lib/config.py")
    pattern = str(Path.home() / ".claude" / "plugins" / "cache" / "*" / "cleanread" / "*" / "scripts")
    for cand in reversed(sorted(glob.glob(pattern))):
        if (Path(cand) / "lib" / "config.py").is_file():
            return Path(cand)
    raise FileNotFoundError("could not locate cleanread plugin scripts/ (set CLEANSCAN_CLEANREAD)")
...
#### 返回 cleanread 的 (config, db) 模块（加载一次并缓存） [@380kkm 2026-06-05] ####
def cleanread_lib() -> tuple[ModuleType, ModuleType]:
    """按文件路径以别名化的名字加载。"""
    if "config" not in _MR:
        libdir = cleanread_scripts_dir() / "lib"
        _MR["config"] = _load_module("cleanread_config", libdir / "config.py")
        _MR["db"] = _load_module("cleanread_db", libdir / "db.py")
    return _MR["config"], _MR["db"]


#### 一个 cleanread 存储库：其 cleanread/ 目录、db、别名与源码根 [@380kkm 2026-06-05] ####
...
#### 列出 cleanread hub 中注册的所有存储库 [@380kkm 2026-06-05] ####
def list_stores() -> list[StoreInfo]:
    mr_config, _ = cleanread_lib()
    out = [
...
def resolve(store: str | None = None, root: str | None = None) -> StoreInfo:
    """委托给 cleanread 的 ``resolve_project``；裸别名（非已存在的路径）与 hub 做匹配。"""
    if store:
        sp = Path(store)
        # 当作 hub 别名
        if not sp.exists():
...
#### cleanread source.db 的只读句柄（files/symbols/edges/meta） [@380kkm 2026-06-05] ####
class Store:
    ...
    def __init__(self, db_path: Path | str):
        ...
        if not self.db_path.is_file():
            raise FileNotFoundError(f"no cleanread index db at {self.db_path}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap = argparse.ArgumentParser(prog="stores.py", description="cleanscan store access smoke test")
    ...
    try:
        print(f"# cleanread scripts: {cleanread_scripts_dir()}", file=sys.stderr)

...
# ///
# audience: internal
# cleanscan.lib.render.html
"""cleanscan.lib.render.html — 自包含的交互式 HTML 视图（sigma.js）。

模块级不变量：输出逐字节恒等——`assets/*.js`（graphology、sigma、bootstrap、module_addon）
被内联写入每个 HTML 文件，同一图两次渲染产出完全相同的字节；
受门控的 HIDDEN、MODULES、N 路分区 const 在参数省略时一律不写入，输出与不带这些参数时一致。
图到视图数据的纯转换在 `htmlbake.py`，本文件持 HTML、CSS 外壳、素材内联与 `to_html` 编排，
并转发导出 `_importance` 以保 `render` 包对外表面不变。
"""
...
def to_html(g: Graph, title: str = "cleanscan dependency slice", view: str = "both",
    ...
    渲染器把已排序的列表写入受门控的 ``const HIDDEN=``，为 ``None`` 时省略该行。
    ``module_of`` 与 ``modules_meta``（可选，可折叠的模块与符号之间的商图视图）：两者都给出时，
    每个节点获得受门控的 ``attrs['module']``（带侧前缀的模块 id），并写入已排序的
    ``const MODULES=`` 列表，bootstrap 渲染默认全折叠的商图；为 ``None`` 时不写入这两项。

    ``module_mode``、``zone_matrix``、``modules_list``（可选，N 路模块分区）：``module_mode``
    为真时，跨 cluster 的边按 cross 上色，并写入受门控的 ``const MODULE_MODE``、``ZONE_MATRIX``
    、``MODULE_LIST``，bootstrap 据此装配 N 路视图选择器和区矩阵面板；三者均为 ``None``
    或缺省时一律省略，输出与不带这些参数时逐字节一致（与 HIDDEN/MODULES 同一门控纪律）。
    """
    mod_mode = bool(module_mode)

    # 着色键、重要性、节点、边属性：纯数据转换

...
# ///
# audience: internal
# cleanscan.lib.render.textfmt
"""cleanscan.lib.render.textfmt —— Graph 和 Metrics 的确定性纯文本视图。"""
...
#### 生成有界状态提示行（截断或深度封顶） [@380kkm 2026-06-05] ####
...
def to_text(g: Graph) -> str:
    """首行统计节点和边数，有界提示行紧跟其后，随后按 id 排序逐行列出节点标签，边界节点行尾标注 ``(+N 越界)``。"""
...
def metrics_text(m: analyze.Metrics) -> str:
    """首行汇总节点、边、环、桥、切点和分层计数；随后有界提示、最不稳定与最被依赖节点；再列出环、桥、切点及不稳定度前 5 的节点。"""
    s = m.summary
    lines = [

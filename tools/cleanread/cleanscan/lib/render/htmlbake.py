# audience: internal
# cleanscan.lib.render.htmlbake
"""cleanscan.lib.render.htmlbake — 把图生成为 HTML 视图的纯数据（确定性，无 IO）。

本模块只把 `Graph` 折算成 sigma.js 渲染所需的素材：重要性信号、布局种子坐标、
着色键、节点与边的属性列表，以及写入 `<script>` 的 `const` 块。所有产出按 id 排序，
两次生成逐字节一致。HTML 与 CSS 外壳以及素材内联留在 `html.py`，本模块不做 IO。
"""
...
#### 取节点的展示路径：优先证据路径，否则取 `label` 或 `id` [@380kkm 2026-06-05] ####
def _path_of(n) -> str:
...
#### 选着色键与配色：分区图按分区着色，有簇时按簇着色，否则按 kind 着色 [@380kkm 2026-06-05] ####
...
def _bake_nodes(g: Graph, imp: dict[str, dict], zoned: bool, clustered: bool,
    ...
    for i, n in enumerate(n_sorted):
        ...
        # 多模块分区模式下从节点属性取模块名写入 attrs['module']
        elif mod_mode:
...
#### 烘焙边属性列表：关系、置信度、bridge 标记、跨越着色 [@380kkm 2026-06-05] ####
...
#### 烘焙 const 块：DATA 及按需输出的 HIDDEN、MODULES、多模块分区 [@380kkm 2026-06-05] ####

...
# ///
# audience: internal
# cleanscan.lib.render.graphfmt
"""cleanscan.lib.render.graphfmt —— 确定性的 mermaid、graphviz dot 视图。"""
...
def to_mermaid(g: Graph) -> str:
    """节点、边按 id 排序；截断、深度封顶时以注释行说明，边界节点附加 ``+N⤳`` 表示越界依赖数。"""
    lines = ["flowchart TD"]
    if g.truncated:
...
def to_dot(g: Graph) -> str:
    ...
    lines = ["digraph cleanscan {", "  rankdir=LR;"]
    if g.truncated:

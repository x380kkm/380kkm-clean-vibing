...
# audience: internal
# cleanscan.lib.deps
"""cleanscan.lib.deps —— 基于存储库的真实跨文件依赖提取。

cleanread 不存任何 import 图，且只在「单个文件内」解析继承。本模块补上 cleanscan
构建真实跨文件边所需的两件事：
  ...
  * 把 cleanread 边的 ``dst_name``（extends/implements/references，cleanread 只在
...
def resolve_import(store: "stores.Store", ref: ImportRef, from_path: str | None = None,
    ...
    """走 :class:`PathIndex` 的精确、后缀、基名匹配（未传时按 Store 取缓存索引）；外部依赖或无法解析时返回 None。"""
...
def resolve_edge_targets(store: "stores.Store", dst_name: str,
    ...
    """按精确名跨整个存储库解析；``len(result)`` 即歧义度（0 = 外部或不存在）。

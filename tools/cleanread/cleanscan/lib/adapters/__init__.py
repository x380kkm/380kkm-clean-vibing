...
# ///
# audience: internal
# cleanscan.lib.adapters
"""cleanscan.lib.adapters —— 可插拔的依赖来源适配器。

一个 :class:`SourceAdapter` 把 cleanread 存储库转换成 cleanscan 的图原语：
``seed_nodes``（把种子解析成起始节点）与 ``neighbors``（产出进出某节点的
...
#### 在 cleanread 存储库上产出 cleanscan 图原语的来源协议 [@380kkm 2026-06-05] ####
@runtime_checkable
...
#### v1 适配器：由代码 imports 或 includes 派生 file 到 file 依赖边 [@380kkm 2026-06-05] ####
class CodeAdapter:
    name = "code"

    #### 把种子解析成起始 file 节点，依次尝试精确路径、符号名、模糊名、全文检索 [@380kkm 2026-06-05] ####
    def seed_nodes(self, store: "stores.Store", seed: str, alias: str | None = None,
                   max_seeds: int = 25) -> list[Node]:
        seed = seed.strip()
        out: dict[str, Node] = {}
...
class SymbolAdapter:
    ...
    #### 把种子解析成处于 TARGET 区的符号节点 [@380kkm 2026-06-05] ####
    def seed_nodes(self, store: "stores.Store", seed: str, alias: str | None = None,
                   max_seeds: int = 25) -> list[Node]:

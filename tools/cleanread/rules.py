...
# audience: internal
# rules
"""cleanread override 规则的编写、检视 CLI，并把引擎重导出供 enrich import。

引擎本体（纯函数 `apply_rules`、`load_rules`、`validate_rules_doc` 与预设解析）位于
...
CLI:  rules.py init|list|validate [<alias>] [--root PATH] [--rules PATH]
      （需要 DB 的 preview|apply 由 enrich 驱动；此处仅为占位实现。）
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="rules.py",
        description="cleanread enrichment override-rules: author/inspect/validate.",
    )
    ...
    def add_common(p):
        ...
        p.add_argument("--store", default=None, help="explicit cleanread store dir (default: discover)")
    ...
    p_init = sub.add_parser("init", help="write a starter <root>/.cleanread/rules.json")
    add_common(p_init)

...
# audience: internal
# trace
"""cleanread L3 —— 查询轨迹存储库的命令行工具（仅依赖标准库）。
...
没有 `~/.cleanread` 主目录；存储库通过从 cwd 向上回溯发现
（或经由 --store / --root / CLEANREAD_STORE 指定）。
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="trace.py", description="cleanread L3 trace store")

# audience: internal
# lib.rules_engine
"""cleanread L2 富化的 override 规则引擎与预设（仅依赖标准库）。
...
编辑的 override 层中，作为一道变换施加：tree-sitter 抽取出原始符号后，先经 apply_rules 变换，
再写入 symbols 与 edges 表。

没有任何规则时，apply_rules 不改动任何行，行为与施加规则前完全一致（向后兼容）。引擎是纯函数
（无数据库、无输入输出），因此可被单元测试，也可从 enrich 驱动。
...
# 内置预设所在目录:<repo>/presets/<name>.rules.json(本文件位于 <repo>/scripts/lib/rules_engine.py)
BUILTIN_PRESET_DIR = Path(__file__).resolve().parent.parent.parent / "presets"
...
def apply_rules(rows, edges, content_by_file_id, rules):
    """在副本上把合并后的规则（预设 + 项目，按顺序）逐条施加到符号行。

    入参 rows 是符号行 dict 列表;edges 是通过 _local 引用 rows 的边列表;
    content_by_file_id 是 file_id 到整文件文本的映射(供 rename 重新切片)。
    返回 (rows, edges, provenance),其中 provenance 为 {rule_id: [被改动的 local id]}。
    rules == [] 时原样返回输入。
    """
    # 在副本上工作
...
#### 读取并 JSON 解析一个规则或预设文件 [@380kkm 2026-06-05] ####

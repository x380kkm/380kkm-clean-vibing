...
# audience: internal
# enrich_treesitter
"""cleanread L2 —— 基于 tree-sitter 的 symbol/edge 富化。

读取一个 project 的 <root>/.cleanread/source.db 中的 ``files`` 表，按语言用
...
原始 tree-sitter 抽取之后，有一个可选的 project 级覆盖规则处理步骤
（spec 第 16 节），用于纠正 codebase 特有的写法（例如 Unreal 导出宏被误读成类名）。
规则存在 <root>/.cleanread/rules.json，经 rules.py 中的纯引擎施加；符号会获得
``attrs``（json）+ ``provenance``（json）。无规则文件（且无 --rules）-> 与基础行为
完全一致（向后兼容）。

本脚本幂等：先清空已有的 ``symbols``/``edges`` 再重填（全量重建）。写入
...
本模块是 ``enrich`` 包的外层入口（实现已拆入该包）：具体实现落在 ``enrich`` 包里

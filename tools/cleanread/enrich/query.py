...
def _query_symbols(file_id: int, tree, src: bytes, query, lang: str) -> list[dict]:
    ...
    #### 收集每个 `@def.*` 捕获的 (start_byte, end_byte, kind, name)，并记录对应的 head [@380kkm 2026-06-05] ####
...
# 扩展贡献的额外 .scm 目录（由 Registry.add_scm_dir 追加）
EXTRA_SCM_DIRS: list[Path] = []


#### 从一个目录加载 *.scm，按 stem 覆盖写入 specs，读取失败时跳过 [@380kkm 2026-06-05] ####
...
#### 按三个来源加载 lang 到 .scm 文本的映射：内置预设、扩展目录、项目覆盖，后者优先级依次更高 [@380kkm 2026-06-05] ####
def _load_query_specs(root) -> dict[str, str]:
    ...
    if root is not None:
        # 项目覆盖最后加载，确保始终胜出
        _load_scm_dir(specs, Path(root) / ".cleanread" / "queries")
    return specs
#### /按三个来源加载 lang 到 .scm 文本的映射 ####


#### 将捕获的类型或名字简化为裸标识符，用于按名称解析 [@380kkm 2026-06-05] ####

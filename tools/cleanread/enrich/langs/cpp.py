...
#### 尽力提取一个 cpp 定义节点的声明符或名字 [@380kkm 2026-06-05] ####
...
#### 取一个函数返回类型和形参声明中的具名类型（去重） [@380kkm 2026-06-05] ####
...
#### /取一个函数返回类型和形参声明中的具名类型 ####


#### 为一个预处理分支生成可读标签（被测的宏或条件表达式） [@380kkm 2026-06-05] ####
...
def _walk_cpp(node: Node, src: bytes, pend: Pending, parent_local: int | None) -> None:
    ...
    if t in _CPP_DEFS:
        ...
        # uses_type：函数返回类型和形参中的具名类型是该函数的依赖
        ...
        # 不改 cur_parent：ifdef 内的定义仍归属外围 scope

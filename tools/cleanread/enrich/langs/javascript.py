...
#### 取 lexical_declaration 绑定的箭头函数或函数表达式的名字 [@380kkm 2026-06-05] ####
def _js_lexical_fn_name(node: Node, src: bytes) -> str | None:
    ...
    return None
#### /取 lexical_declaration 绑定的箭头函数或函数表达式的名字 ####
...
def _walk_javascript(node: Node, src: bytes, pend: Pending, parent_local: int | None) -> None:
    ...
    if t == "class_declaration":
        ...
        if heritage is not None:
            # class_heritage 形如 `extends <expr>`（可附带 TypeScript 的 implements 子句）

...
#### 解析单个文件，返回 rows 和 edges 两个字典列表 [@380kkm 2026-06-05] ####
def _extract_file(file_id: int, content: str, lang: str, parser: Parser,
    ...
    if lang in HAS_WALKER:
        ...
        edges: list[dict] = []
        # 由词法包含关系生成的 contains 边（父节点指向子节点）
        for local_idx, r in enumerate(pend.rows):
        ...
        # 可选的 references 边，尽力匹配文件内引用，默认关闭
        if do_refs:
        ...
        # `@dep` 生成的 wire 边，归属到最内层外围的 `@def` 符号
...
#### 尽力生成 references 边，返回字典列表格式 [@380kkm 2026-06-05] ####

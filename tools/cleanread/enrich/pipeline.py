...
#### 把一组 files.path 解析为 file_id 列表（空集合返回空列表） [@380kkm 2026-06-16] ####
def _ids_for_paths(conn, paths: set[str]) -> list[int]:
    if not paths:
        return []
    qs = ",".join("?" for _ in paths)
    return [r[0] for r in conn.execute(
        f"SELECT id FROM files WHERE path IN ({qs})", tuple(paths)).fetchall()]
#### /把路径解析为 file_id ####


#### 把一个 --files 输入路径归一到 files.path 的存储形态（root 相对正斜杠） [@380kkm 2026-06-16] ####
def _normalize_rel(p: str, root: Path) -> str:
    """统一正斜杠、去 ./ 前缀（Path 规整自动完成）；绝对路径折成相对 root。与 index_build 的
    存储归一（relative_to(root).as_posix()）对齐。"""
    pp = Path(p.strip().replace("\\", "/"))
    if pp.is_absolute():
        try:
...
def enrich(cfg: config.ProjectConfig, langs: list[str], do_refs: bool,
           ...
           preview: bool = False, only_paths: set[str] | None = None) -> dict:
    """preview=True 时只计算变换并收集变换前后的差异，不写入数据库。
    only_paths 非 None 时只重新提取这些文件的符号与边（增量模式）：先删除它们已有的 symbols 与 edges 再重新写入，其余文件不动；
    为 None 时重新提取全部文件（全量）。"""
    ...
    #### 解析 c 系语言的 parse 前宏剥离配置（cleanread.json macro_strip；缺省即开启） [@380kkm 2026-06-05] ####
    ...
    try:
        ...
        if not preview:
            #### 清除既有 enrich 结果：增量(only_paths 非 None)只清指定文件，全量清全部 [@380kkm 2026-06-16] ####
            # 用 is not None 区分「显式选 0 个文件」（空集，删 0）与「全量」（None），空集绝不退化为全量清库
            if only_paths is not None:
                ids = _ids_for_paths(conn, only_paths)
                for fid in ids:
        ...
        if only_paths is not None:
            qs = ",".join("?" for _ in only_paths) or "NULL"
            rows = conn.execute(
                f"SELECT id, path, ext, content FROM files WHERE path IN ({qs})",
                tuple(only_paths)).fetchall()
        else:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description="cleanread L2 tree-sitter symbol/edge enrichment.",
    )
    parser.add_argument("--root", default=None, help="source tree root (default: store's parent)")
    parser.add_argument("--store", default=None,
                        help="explicit cleanread store dir (default: discover from cwd)")
    ...
    parser.add_argument("--rules", default=None,
                        help="override-rules path (default <root>/.cleanread/rules.json "
    ...
    parser.add_argument("--files", default=None,
                        help="comma-separated relative paths: incrementally re-enrich only "
                             "these files (delete their symbols/edges and re-extract), leave "
                             "the rest untouched")
    ...
    if args.files is not None:
        root = Path(cfg.root)
        only_paths = {_normalize_rel(p, root) for p in args.files.split(",") if p.strip()}
        if not only_paths:
            parser.error("--files 未含有效路径")
        # 未命中索引的路径给出告警（防止路径拼错后误以为已重新提取、实际什么也没发生）
        conn = db.connect(Path(cfg.db_path))
        try:
            in_index = {r[0] for r in conn.execute(
                "SELECT path FROM files WHERE path IN ({})".format(
                    ",".join("?" for _ in only_paths)), tuple(only_paths)).fetchall()}
        finally:
            conn.close()
        for miss in sorted(only_paths - in_index):
            print(f"warning: --files 路径不在索引中，已跳过: {miss}", file=sys.stderr)
        if not (only_paths & in_index):
            print("error: --files 给的路径无一在索引中（检查拼写/大小写/是否被 ignore）",
                  file=sys.stderr)
            return 2

    try:
        stats = enrich(cfg, langs, do_refs=args.refs, rules_path=args.rules,
                       no_rules=args.no_rules, preview=args.rules_preview, only_paths=only_paths)
    except SystemError as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 2
